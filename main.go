...
func attack(charName, charClass string) string {
	...
	case "healer":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, randInt(1, 3))
	default:
...
func chooseCharClass() string {
	...
	for strings.ToLower(approveChoice) != "y" {
		fmt.Print("Выбери персонажа: Воитель — warrior, Маг — mage, Лекарь — healer: ")
		fmt.Scanf("%s\n", &charClass)
		...
		fmt.Print("Нажми (Y), чтобы подтвердить выбор, или другую клавишу — выбрать заново: ")
		fmt.Scanf("%s\n", &approveChoice)
	}
